...
class XRAYData2StageTestSet(data.Dataset):
    def __init__(self, root, list_path, crop_size=(512, 512), mean=(128, 128, 128), scale=False, mirror=False):
        self.root = root
        self.list_path = list_path
        self.crop_h, self.crop_w = crop_size
        self.mean = mean
...
class XRAYDataTestSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = cv2.imread(datafiles["img"], 0)
