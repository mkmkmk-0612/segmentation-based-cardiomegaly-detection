...
class SpatialGate(nn.Module):
    ...
    '''
    def deconv_add(self, x, y):
        _,k,H,W = y.size()
        _,a,b,c = x.size()
        #z=nn.ConvTranspose2d(x, size=(H,W),3,stride=1,padding=1)
        #z=nn.ConvTranspose2d(k, k,3,stride=1,padding=1)
        return nn.ConvTranspose2d(x, k,3,stride=1,padding=1)+ y ##problem
    '''
...
class U_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        d5 = self.Up5(x5)
        #print('d5.shape Up : ', d5.shape)
        #x4 = self.Att4(x4)
        d5 = torch.cat((x4, d5), dim=1)
        #print('d5.shape cat : ', d5.shape)
        d5 = self.Up_conv5(d5)
        #print('d5.shape : ', d5.shape)
        #d5 = self.Att4(d5)

        d4 = self.Up4(d5)
        #print('d4.shape Up : ', d4.shape)
        #x3 = self.Att3(x3)
        d4 = torch.cat((x3, d4), dim=1)
        #print('d4.shape cat : ', d4.shape)
        d4 = self.Up_conv4(d4)
        #print('d4.shape : ', d4.shape)
        #d4 = self.Att3(d4)

        d3 = self.Up3(d4)
        #print('d3.shape Up : ', d3.shape)
        #x2 = self.Att2(x2)
        d3 = torch.cat((x2, d3), dim=1)
        #print('d3.shape cat : ', d3.shape)
        d3 = self.Up_conv3(d3)
        #print('d3.shape : ', d3.shape)
        #d3 = self.Att2(d3)

        d2 = self.Up2(d3)
        #print('d2.shape Up : ', d2.shape)
        #x1 = self.Att1(x1)
        d2 = torch.cat((x1, d2), dim=1)
        #print('d2.shape cat : ', d2.shape)
        d2 = self.Up_conv2(d2)
